...
export default function PlanList() {
  ...
  const {
    ...
    queryKey: ["plans"],
    queryFn: ({ pageParam = 1 }) => getPlans(pageParam, 12),

    initialPageParam: 1,

    getNextPageParam: (lastPage) => {
      const response = lastPage.data;
      if (!response.hasMore || response.page >= response.totalPages) {
        return undefined;
      }
      return response.page + 1;
    },
  });
  console.log("data from the paln: ", data);
  ...
  const plans = data?.pages.flatMap((page) => page.data.plans) ?? [];
